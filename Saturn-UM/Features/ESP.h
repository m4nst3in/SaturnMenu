...
inline void DrawPreviewBox(const ImVec2& startPos, const ImVec2& endPos, ImColor boxColor, float rounding, float thickness, bool filled) {
    ...
    inline ImVec2 ClampToBounds(const ImVec2& p, float minX, float minY, float maxX, float maxY)
    {
        return ImVec2(min(max(p.x, minX), maxX), min(max(p.y, minY), maxY));
    }

    // Draw an equilateral triangle oriented by angle
    inline void DrawArrowAt(ImDrawList* dl, const ImVec2& pos, float angleRad, float size, ImU32 col)
    {
    ...
    inline void RenderOutOfFOVArrow(const CEntity& local, const CEntity& entity)
    {
