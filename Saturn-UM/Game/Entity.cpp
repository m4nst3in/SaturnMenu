...
bool EntityBatchProcessor::ProcessCoreEntityData(
	...
	for (auto& [entityIndex, entity] : entities) {
		...
		if (currentOffset + ENTITY_DATA_SIZE > buffer.size()) {
			break; // Stop processing if buffer is too small
		}

		// Extract controller data (6 fields)
		if (currentOffset + sizeof(int) > buffer.size()) break;
		...
		if (currentOffset + MAX_PATH > buffer.size()) break;
		...
		if (currentOffset + sizeof(INT64) > buffer.size()) break;
		...
		if (currentOffset + sizeof(DWORD64) > buffer.size()) break;
		...
		if (currentOffset + sizeof(float) > buffer.size()) break;
		...
		if (currentOffset + sizeof(C_UTL_VECTOR) > buffer.size()) break;
...
bool EntityBatchProcessor::ProcessWeaponData(
	...
	for (size_t i = 0; i < weaponAddresses.size(); ++i) {
		if (i < weaponAddresses.size() && weaponAddresses[i] != 0) {
	...
	for (size_t i = 0; i < validWeaponIndices.size(); ++i) {
		...
		// Safety check: ensure we don't exceed buffer bounds
		if (bufferOffset + WEAPON_DATA_SIZE > buffer.size()) {
			break;
		}

		// Extract ammo
		if (bufferOffset + sizeof(int) > buffer.size()) break;
		int ammo;
	...
	// Note: weaponAddresses.size() may be less than entities.size() if ProcessCoreEntityData 
	// didn't process all entities due to buffer size issues
	for (size_t i = 0; i < entities.size(); ++i) {
		if (i < weaponAddresses.size() && weaponAddresses[i] == 0) {
			// i is already checked to be < entities.size() in the loop condition
			entities[i].second.Pawn.Ammo = 0;
			entities[i].second.Pawn.WeaponName = "Weapon_None";
		} else if (i >= weaponAddresses.size()) {
			// This entity wasn't processed in ProcessCoreEntityData, set defaults
			entities[i].second.Pawn.Ammo = 0;
			entities[i].second.Pawn.WeaponName = "Weapon_None";
		}
	}
...
bool EntityBatchProcessor::ProcessDependenciesData(
	...
	for (size_t i = 0; i < entities.size(); ++i) {
		//if (i < weaponDataAddresses.size() && weaponDataAddresses[i] != 0) {
		...
		//}
		if (i < cameraAddresses.size() && cameraAddresses[i] != 0) {
	...
	for (const auto& entityIndex/*[entityIndex, isMaxAmmo]*/ : requestMap) {
		...
		if (bufferOffset + sizeof(int) > buffer.size()) {
			break;
		}
		int value;
