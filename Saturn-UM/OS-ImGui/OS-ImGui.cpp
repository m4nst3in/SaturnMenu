...
namespace OSImGui
{
    ...
    bool OSImGui::LoadTextureFromMemoryIcon(unsigned char* Memory, UINT size, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height)
    {
        int image_width = 0;
        int image_height = 0;
        unsigned char* image_data = stbi_load_from_memory(Memory, size, &image_width, &image_height, NULL, 4);
        ...
        auto px = [&](int x, int y) {
            ...
            return ImVec4(image_data[idx] / 255.0f, image_data[idx + 1] / 255.0f, image_data[idx + 2] / 255.0f, image_data[idx + 3] / 255.0f);
        };
        ImVec4 corners[4] = { px(0,0), px(image_width-1,0), px(0,image_height-1), px(image_width-1,image_height-1) };

        // Média dos cantos com alfa alto como cor de fundo
        float sum_r = 0.f, sum_g = 0.f, sum_b = 0.f; int count = 0;
        for (auto &c : corners) { if (c.w > 0.8f) { sum_r += c.x; sum_g += c.y; sum_b += c.z; count++; } }
        ImVec4 bg = count ? ImVec4(sum_r / count, sum_g / count, sum_b / count, 1.f) : corners[0];

        const int tolerance = 24; // tolerância para identificar fundo
        auto near_bg = [&](unsigned char r, unsigned char g, unsigned char b) {
            return (abs((int)r - (int)(bg.x * 255)) <= tolerance &&
                    abs((int)g - (int)(bg.y * 255)) <= tolerance &&
                    abs((int)b - (int)(bg.z * 255)) <= tolerance);
        };

        // Processar: tornar fundo transparente e deixar conteúdo branco
        ...
        for (int i = 0; i < total; ++i)
        {
            ...
            unsigned char a = image_data[idx + 3];

            if (a == 0) continue; // já transparente

            if (near_bg(r, g, b))
            {
                image_data[idx + 3] = 0; // remover fundo
            }
            else
            {
                // conteúdo do ícone fica branco preservando alfa
                image_data[idx] = 255;
        ...
        desc.Width = image_width; desc.Height = image_height; desc.MipLevels = 1; desc.ArraySize = 1;
        ...
        ID3D11Texture2D* pTexture = NULL;
        D3D11_SUBRESOURCE_DATA subResource; subResource.pSysMem = image_data; subResource.SysMemPitch = image_width * 4; subResource.SysMemSlicePitch = 0;
        g_Device.g_pd3dDevice->CreateTexture2D(&desc, &subResource, &pTexture);
        ...
        g_Device.g_pd3dDevice->CreateShaderResourceView(pTexture, &srvDesc, out_srv);
        pTexture->Release();
        *out_width = image_width; *out_height = image_height;
        stbi_image_free(image_data);
        return true;
    }
