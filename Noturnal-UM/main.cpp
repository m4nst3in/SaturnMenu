...
|'....|'  '|..'|'  '|.'  '|..'|. .||.    .||. ||. 
    ...
    MenuConfig::path = MenuConfig::docPath + "\\Noturnal";
	try
	{
...
CONNECT_KERNEL://CONNECT_KERNEL
    Log::Info("Trying to connect to Noturnal-KM...");
    if (memoryManager.ConnectDriver(L"\\\\.\\Noturnal-KM"))
	{
		Log::PreviousLine();
        Log::Fine("Connected to Noturnal-KM!");
	}
	else
	{
		Log::PreviousLine();
        Log::Error("Failed to connect to Noturnal-KM", mapped, mapped);
		Log::Info("Triggered auto-map protocol");
		Log::Info("Looking for kernel mapper...");

        std::string mapperExe = "";
        if (fs::exists("Noturnal-KM-kernel.exe"))
            mapperExe = "Noturnal-KM-kernel.exe";
...
UPDATE_OFFSETS://UPDATE_OFFSETS
	...
	Log::PreviousLine();
        Log::Fine("VAC pwned!");
            Log::Fine("Noturnal loaded succesfully!");
